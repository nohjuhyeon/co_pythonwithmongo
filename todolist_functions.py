...
def Data_insert(collection, data):
    ...
    # 데이터 입력
    collection.insert_many(data)           # hint   insert_one -> insert_many로 변경
...
def Todos(user_id, collection1, collection2):
    ...
    result_todo = collection1.find({})           # hint     user_id -> collection1로 변경
    count = 1
    for i in result_todo:
        print("{}. {}".format(count, i["title"]), end=" ")           # hint   format()안에 있는 필요없는 인수 제거
        count+= 1           # hint  3 -> 1로 변경
    print("")
...
# 종료 여부 입력 function
def End(collection3, collection1, collection2):           # hint        user_id 추가
    user_end = 'q'           # hint     # q -> x 로 변경
    while True:
        # c 입력 시 Todos() 다시 실행
        if user_end == "c":
            print("")
